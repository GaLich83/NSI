...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Подменю_Левая = Форма.Элементы.Добавить(ПодменюИмя + СуффиксЛевых, Тип("ГруппаФормы"), Форма.Элементы.ГруппаОтправить_Левая);
	ЗаполнитьЗначенияСвойств(Подменю_Левая, Подменю, "Вид, Заголовок");
	
	ЭлементСоздать = Форма.Элементы.Добавить(КомандаСоздать.Имя + СуффиксЛевых, Тип("КнопкаФормы"), Подменю_Левая);
	ЭлементСоздать.ИмяКоманды = КомандаСоздать.Имя;
	ЭлементСоздать.ТолькоВоВсехДействиях = Ложь;
	
	ЭлементПрисоединить = Форма.Элементы.Добавить(КомандаПрисоединить.Имя + СуффиксЛевых, Тип("КнопкаФормы"), Подменю_Левая);
	ЭлементПрисоединить.ИмяКоманды = КомандаПрисоединить.Имя;
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	МассивРеквизитов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		"Имя, ПолноеИмя, КлючВарианта, ЭтоВариант, СКД, Метаданные, Объект, " +    // общие параметры
	...
	Если ПараметрыОтчета.ЭтоВариант Тогда
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыОтчета.Отчет, "Отчет, КлючВарианта");
		ЗаполнитьЗначенияСвойств(ПараметрыОтчета, ЗначенияРеквизитов, "КлючВарианта");
		
		РезультатПодключения = ВариантыОтчетов.ПодключитьОтчетОбъект(ЗначенияРеквизитов.Отчет);
		Если Не РезультатПодключения.Подключен Тогда
			...
			Возврат ПараметрыОтчета.Инициализирован;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(ПараметрыОтчета, РезультатПодключения, "Имя, ПолноеИмя, Метаданные, Объект");
...
Процедура ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурнала = Неопределено, КомментарийСПараметрами = "", 
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ТекстДляПользователя = ТекстДляПользователя + Символы.ПС + ОтчетыКлиентСервер.КраткоеПредставлениеОшибкиФормированияОтчета(ИнформацияОбОшибке);
	ИначеЕсли ТипЗнч(ИнформацияОбОшибке) = Тип("Строка") Тогда
...
Функция ПараметрыЗаписиТабличногоДокументаВФормат(Формат)
	...
	Иначе 
		// СРыба" для всех форматов, добавленных при внедрении, обработчик сохранения
