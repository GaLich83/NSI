...
Функция ВариантыАвтоподбора(Текст, Родитель, Уровни, ДополнительныеПараметры) Экспорт 
	...
	Если ПустаяСтрока(Источник) Тогда
		...
		Если УровниЗапроса.Найти(7) = Неопределено Тогда
			СписокАвтоподбораЧастиНаселенногоПункта(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры);
		Иначе
...
// Заполнение данных для автоподбора из загруженных данных для населенного пункта
//
Процедура СписокАвтоподбораЧастиНаселенногоПункта(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Родитель) И УровниЗапроса.Количество() = 1 Тогда
		Уровень = УровниЗапроса[0];
		ТекстОтбора = ?(Уровень > 2, " И РодительскийОбъект.КодОкруга = АдресныйОбъект.КодОкруга", "");
		ТекстОтбора = ТекстОтбора + ?(Уровень > 3, " И РодительскийОбъект.КодРайона = АдресныйОбъект.КодРайона ", "");
		ТекстОтбора = ТекстОтбора +  ?(Уровень > 4, " И РодительскийОбъект.КодГорода = АдресныйОбъект.КодГорода  ", "");
		ТекстОтбора = ТекстОтбора +  ?(Уровень > 5, " И РодительскийОбъект.КодВнутригородскогоРайона = АдресныйОбъект.КодВнутригородскогоРайона ", "");
		ТекстОтбора = ТекстОтбора +  ?(Уровень > 6, " И РодительскийОбъект.КодНаселенногоПункта = АдресныйОбъект.КодНаселенногоПункта ", "");
		...
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК РодительскийОбъект
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект
		|		ПО РодительскийОбъект.КодСубъектаРФ = АдресныйОбъект.КодСубъектаРФ " + ТекстОтбора + "
		|ГДЕ 
		|	АдресныйОбъект.Наименование ПОДОБНО &НачалоФразы
		|	И РодительскийОбъект.Идентификатор = &Идентификатор
		|	И АдресныйОбъект.Уровень = " + Строка(Уровень) + "
		|УПОРЯДОЧИТЬ ПО
		|	АдресныйОбъект.Уровень
		|ИНДЕКСИРОВАТЬ ПО
		|	АдресныйОбъект.Уровень,
	...
	Для Каждого Строка Из Запрос.Выполнить().Выгрузить() Цикл
		...
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.Наименование, Строка.Сокращение);
		Если УровниЗапроса.Найти(6) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.НаселенныйПунктНаименование, Строка.НаселенныйПунктСокращение);
		КонецЕсли;
		Если УровниЗапроса.Найти(5) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ВнутригородскойРайонНаименование, Строка.ВнутригородскойРайонСокращение);
		КонецЕсли; 
		Если УровниЗапроса.Найти(4) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ГородНаименование, Строка.ГородСокращение);
		КонецЕсли;
		Если УровниЗапроса.Найти(3) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.РайонНаименование, Строка.РайонСокращение);
		КонецЕсли;
		Если УровниЗапроса.Найти(2) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.ОкругНаименование, Строка.ОкругСокращение);
		КонецЕсли;
		Если УровниЗапроса.Найти(1) <> Неопределено Тогда
			ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.СубъектРФНаименование, Строка.СубъектРФСокращение);
		КонецЕсли;
		СтрокаРезультата.Представление = Представление;
