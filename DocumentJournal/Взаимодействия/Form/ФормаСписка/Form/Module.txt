...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Иначе
		...
		Если ВыборКорректен(Элемент.Имя,Истина) И
			ВзаимодействиеДляКоторогоСформированПредпросмотр <> Элементы.Список.ТекущиеДанные.Ссылка Тогда
...
Процедура ЗаполнитьПанельКонтактов()
	...
	Если ТолькоЗначимыеКонтакты Тогда
		...
		СтрокаПоКоторойИщем = Прав(ТекстЗапросаПоОтбору, СтрДлина(ТекстЗапросаПоОтбору) -  Найти(ТекстЗапросаПоОтбору,"ГДЕ") + 1);
		МассивСтрокУсловия = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаПоКоторойИщем, Символы.ПС,Истина);
		...
		Для Каждого СтрокаУсловия Из МассивСтрокУсловия Цикл
			...
			КонецЕсли;
			Если Найти(СтрокаУсловия, "ЖурналДокументовВзаимодействия") Тогда
				ТекстУсловийПоЖурналуДокументов = ТекстУсловийПоЖурналуДокументов + СтрокаУсловия + Символы.ПС;
			ИначеЕсли  Найти(СтрокаУсловия, "ПредметыВзаимодействий") Тогда
		...
		Для каждого ПараметрЗапроса Из Запрос.Параметры Цикл
			Если Лев(ПараметрЗапроса.Ключ, 1) = "П" Тогда
...
Процедура СкопироватьОтбор(Приемник, Источник, УдалятьПредставлениеГруппы = Ложь, УдалятьНеиспользуемые = Истина, НеВключатьОтборПанелиНавигации = Ложь)
	...
	Для каждого ЭлементОтбораИсточник Из Источник.Элементы Цикл
		...
		Если НеВключатьОтборПанелиНавигации И 
			...
			ЭлементОтбораИсточник.Представление = "ОтборПанелиНавигации" Тогда
