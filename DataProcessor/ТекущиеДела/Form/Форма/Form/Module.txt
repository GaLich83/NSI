...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗапуститьФоновоеЗаданиеНаСервере(ВыполнениеПрерваноНаЗапуске);
	
	ЗагрузитьНастройкиАвтообновления();
	
	Элементы.ФормаНастроить.Доступность = Ложь;
	Если Не ВыполнениеПрерваноНаЗапуске Тогда
...
Процедура ЗапуститьФоновоеЗаданиеНаСервере(ВыполнениеПрервано = Ложь)
	...
	Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
		ТекущиеДелаСлужебный.СписокТекущихДелПользователя(ТекущиеДелаВХранилище);
	Иначе
		ПараметрыПроцедуры = Новый Массив;
		ПараметрыПроцедуры.Добавить(ТекущиеДелаВХранилище);
		ПараметрыПроцедуры.Добавить(ПараметрыСеанса.ПараметрыКлиентаНаСервере);
...
Процедура ЗапуститьОбновлениеСпискаТекущихДел(АвтоматическоеОбновление = Ложь, ОбновитьНезаметно = Ложь)
	...
	ТолькоОбновлениеДел = Не ВыполнениеПрерваноНаЗапуске;
	...
	Если ВыполнениеПрервано Тогда
		Возврат;
	КонецЕсли;
	ВыполнениеПрерваноНаЗапуске = Ложь;
