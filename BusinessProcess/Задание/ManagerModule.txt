...
Процедура УстановитьСостояниеЭлементовФормыЗадачи(Форма) Экспорт
	...
	Если Форма.Элементы.Найти("РезультатВыполнения") <> Неопределено 
		...
		Если ПустаяСтрока(Форма.ЗаданиеРезультатВыполнения) Тогда
			Форма.Элементы.ИсторияВыполнения.Картинка = Новый Картинка();
		Иначе
			Форма.Элементы.ИсторияВыполнения.Картинка = БиблиотекаКартинок.Комментарий;
		КонецЕсли;
	КонецЕсли;
